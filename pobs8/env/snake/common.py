...
def process_observations_each(state, ctrl_agent_index):
    ...
    teammate_map = base_map.copy()
    enemy_map = base_map.copy()
    # for i in range(29):
    ...
    if ctrl_agent_index in list(range(0,30)):
        team_idx = list(range(0, 30))
        enemy_idx = list(range(30, 60))
    else:
        team_idx = list(range(30, 60))
        enemy_idx = list(range(0, 30))
    ...
    for i in range(2,62):
        ...
        for snake_pos in state[i]:
            ...
            if i - 2 in teammate_idx:
                teammate_map[snake_pos[0]][snake_pos[1]][0] = 1
            if i - 2 in enemy_idx:
                enemy_map[snake_pos[0]][snake_pos[1]][0] = 1
    ...
    return np.concatenate((snakes_map, my_head_map, team_head_map, enemy_head_map, beans_map, my_snake_map, teammate_map, enemy_map), axis=-1)

def process_obs_joint(state):
    board_width = state['board_width']
    board_height = state['board_height']
    joint_obs = np.zeros((60, board_height, board_width, 8))
    for i in range(60):
...
def get_reward_joint(reward, state, done):
    ...
    if done:
        if len1>len2:
            ...
            flag = 1
    true_reward = np.zeros((60,))

    reward_0_29 = sum(reward[0:30]) / 30.0
    reward_30_59 = sum(reward[30:60]) / 30.0
    for i in range(0, 30):
        true_reward[i] = reward[i] - reward_30_59
    for i in range(30, 60):
        true_reward[i] = reward[i] - reward_0_29
    ...
    tau = 0.8
    true_reward_2 = np.zeros((60,))
    true_reward_0_29 = sum(true_reward[0:30]) / 30.0
    true_reward_30_59 = sum(true_reward[30:60]) / 30.0
    for i in range(0, 30):
        true_reward_2[i] = true_reward[i] * tau + true_reward_30_59
    for i in range(30, 60):
        true_reward_2[i] = true_reward[i] * tau + true_reward_0_29
...
def get_legal_actions(state):
    legal_actions = [[1 for i in range(4)] for j in range(60)]
    head_pos = []
    board_width = state['board_width']
    board_height = state['board_height']
