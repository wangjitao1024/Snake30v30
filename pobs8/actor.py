...
def run_one_agent(index, args, unknown_args, actor_status):
    ...
    model = ACCNNModel(observation_space = [10,20,8], action_space = 4)
    env = SnakeEnv('snakes-30v30')
    ...
    while True:
        ...
        next_state = obs2pobs(next_state)
        ...
        while not loaded:
            try:
                with open(ckpt_path / latest_file, 'rb') as f:
                    ...
                    # if int(new_model_id) % 1 == 0:
                    #     with open( '/home/wangjt/snakes/rl-framework-baseline-zhaoj2/model-backup/base-model-backup-1-100000/' f'{int(new_model_id)}.pth',
                    #                 'wb') as f:
...
def main():
    ...
    for i in range(args.num_replicas):
        ...
        p.start()
        os.system(f'taskset -p -c {(i+100) % os.cpu_count()} {p.pid}')  # For CPU affinity
